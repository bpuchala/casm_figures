...
root = "square_binary"
clust_json_path = "square_binary/basis_sets/bset.default/clust.json"
x_stop = 40 # larger -> wider figure
figure_inches_wide = 16
figure_inches_tall = 8
...
def plot_background(view_basis, L, df, T, supercell_shift_ijk, cabinet=None, T_conventional=None, axis_labels=['a', 'b', 'c'], hex=False):
    ...
    # cartesian_axes_scale_origin = L @ (supercell_shift_ijk - np.array([2, 0, 1]))
...
for orbit in clust['orbits']:
    ...
    total_shift_ijk = np.array([curr_x, curr_y, 0]) - ijk_min + \
        np.array([0, -T_clust[1][1], 0], dtype=int)
...
plot_background(view_basis, L, _df, T_super, background_shift_ijk,
    cabinet=cabinet, axis_labels=['a', 'a', 'c'])
